...
.count-pills{display:flex;gap:.3rem;flex-wrap:wrap;margin-top:.2rem;}
.count-pill{font-size:.7rem;font-weight:700;padding:.1rem .4rem;border-radius:999px;}
.cp-success{background:#d1fae5;color:#065f46;}
.cp-failed{background:#fee2e2;color:#b00020;}
.cp-review{background:#fef3c7;color:#d97706;}
.cp-pending{background:#f3f4f6;color:#6b7280;}
      ...
      <div class="hist-item">
        <div>
          ...
          <div class="count-pills">
            <?php if ($hasCounts): ?>
              <span class="count-pill cp-success">✅ <?= (int)$run['succeeded'] ?> entered</span>
              <?php if ((int)$run['failed'] > 0): ?>
                <span class="count-pill cp-failed">❌ <?= (int)$run['failed'] ?> failed</span>
              <?php endif; ?>
              <?php if ((int)$run['needs_review'] > 0): ?>
                <span class="count-pill cp-review">⚠️ <?= (int)$run['needs_review'] ?> review</span>
              <?php endif; ?>
            <?php elseif (!empty($run['summary'])): ?>
              <span class="count-pill cp-pending"><?= htmlspecialchars($run['summary']) ?></span>
            <?php endif; ?>
          </div>
        </div>
